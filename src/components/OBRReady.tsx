  ...
  if (isBypassing || isReady) {
    return (
      <>
        {children}
        {isBypassing ? (
          <div className="obr-warning" aria-live="polite">
            <span role="img" aria-label={t('obr.warningLabel')}>
              ⚠️
            </span>
            <span className="obr-warning-tooltip">{t('obr.warningTooltip')}</span>
          </div>
        ) : null}
      </>
    )
